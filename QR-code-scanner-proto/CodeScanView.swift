...
@available(macCatalyst 14.0, *)
public struct MultiScanResult {
    public let results:[FoundCode]
    public let image:UIImage?
}
...
@available(macCatalyst 14.0, *)
public struct CodeScanView: UIViewControllerRepresentable {
    ...
    public var multiCompletion: (Result<MultiScanResult, ScanError>) -> Void
    ...
    public init(
        ...
        multiCompletion: @escaping (Result<MultiScanResult, ScanError>) -> Void,
        ...
        self.multiCompletion = multiCompletion
        self.completion = completion
    }

    public func makeUIViewController(context: Context) -> ScanViewUIController {
        return ScanViewUIController(showViewfinder: showViewfinder, parentView: self)
    }

    public func updateUIViewController(_ uiViewController: ScanViewUIController, context: Context) {
...
struct CodeScannerView_Previews: PreviewProvider {
    static var previews: some View {
        CodeScanView(codeTypes: [.qr], multiCompletion: { result in }) { result in
            // do nothing
        }
    }
}
